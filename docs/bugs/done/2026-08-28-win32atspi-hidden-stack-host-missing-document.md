...
**Seen:** 2026-08-28 — multi-WebView app with `Gtk.Stack` (one visible, one hidden)  
**Library version tested:** **0.5.5** (multi-host + lazy `hosts` init)
...
🔷 On Linux, the same app layout (two WebViews in a `Gtk.Stack`, automation
  loads on the hidden primary while management/Home is visible) **works** —
  dumps match the spider/primary URL and title without forcing a stack switch.
...
A private GTK app hit this with the layout above. Minimal diagnostic exe
(not shipped in the library) confirmed:

- Hidden spider + visible management → **PICK FAIL**
- `restore_primary` equivalent → **PICK OK**

That consumer will short-term call `stack.set_visible_child(primary)` before
dump; the long-term contract should not require it.
...
and [2026-08-27-win32atspi-hosts-null](./2026-08-27-win32atspi-hosts-null.md)
...
`--smoke-stack` in `examples/automation` is the Gtk.Stack repro (hidden primary
load). Interactive: `scripts/run-automation-smoke-stack-interactive.sh`.
Phase A on 0.5.6: both `stack primary document` and `stack secondary document`;
`STACK_SMOKE_PASS`. HBox `--smoke` still `TEST_PASS`.
...
| `--smoke-stack` Phase B | Still includes `stack primary document` (no regression) |
| `--smoke` (HBox) | Still `TEST_PASS` / `a11y_documents>=2` |
| Extents | [2026-08-27-win32atspi-window-extents](./2026-08-27-win32atspi-window-extents.md) unchanged |
| Multi-host | No return to last-wins single host |

Optional follow-up: Phase A with `primary.load_uri("https://www.google.com/?hl=en")`
and assert document name contains `Google`.
...
- `examples/automation/main.vala --smoke-stack` — minimal repro + smoke  
