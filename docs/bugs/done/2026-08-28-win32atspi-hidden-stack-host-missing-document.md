...
**Status:** ✅ fixed in **0.5.7** (parked size kept; Google `--google` smoke `HIDDEN_OK`)  
...
**0.5.6 (partial):** `find_page_document` no longer picks the first
`Chrome_WidgetWin_1` with zero overlap. It scores each UIA Document by page
**title / URI** (from `ICoreWebView2`), then HWND distance to controller bounds,
and `FindAll` under the GTK parent so a parked host still matches. The winning
...
`-30000` (never 1×1) so WebView2 still exposes a UIA Document after a hidden
`load_uri`. Lookup matches by page title/URI and cached overlapping
`Chrome_WidgetWin_1`, not screen-overlap first-wins.
...
Gate: `scripts/run-hidden-stack-smoke-interactive.sh` (`--google`).
