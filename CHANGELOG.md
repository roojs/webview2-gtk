...
- `examples/hello --smoke-policy-ignore`: HTML then a PDF URL; `decide_policy` `ignore()` must cancel (no Edge PDF viewer).

### Fixed

- `PolicyDecision.ignore()` on RESPONSE was a no-op: Stop the document, `load_failed` cancelled, `about:blank` if Edge still commits PDF chrome ([bug](docs/bugs/done/2026-09-22-policy-decision-ignore-noop.md)).
